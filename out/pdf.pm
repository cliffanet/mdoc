...
sub recalc {
    ...
    # Если переполнения @over нет, то делать больше ничего не надо
    @over || return;

    # Теперь вместо себя в списке текущего own надо оставить
...
# ============================================================
#
#       Элементы нового движка - блочные элементы
#
# ============================================================
...
sub draw {
    ...
    my $t = $page->text();
    $t->position($x, $y);
    $t->font($self->{style}->font());
    $t->text(join(' ', map { $_->{str} } @{ $self->{elem} }));
}
