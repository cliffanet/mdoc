...
sub _fnres {
    ...
    foreach my $ref (@fnrefs) {
        ...
        if (!$note) {
            ...
            my $num = @notes + 1;
            # тут необязательно указывать type, как у других элементов, т.к. всё равно fnlist
            # будет сам в цикле разбирать весь свой content, и элементов другого типа в его content
            # быть не может.
            $note = $found{ $e->{code} } = {
                num     => $num,
                id      => 'fn-' . $num,
        ...
        # Добавляем в разрешение обратную ссылку на текст, где встречается сноска
        my $cnt = @{ $note->{refs} } + 1;
        my $id = 'fnref-' . $note->{num};
        $id .= '-' . $cnt if $cnt > 1;
        push @{ $note->{refs} }, $id;

        # новые данные для сноски, полученные в этом проходе (номер/id)
        # эти номера и ID получены после полного прохода документа перечислением всех встреченных сносок
        # по порядку, как они встречаются в документе. Можно было бы уже там начать формировать список
        # этих номеров/id и уже тут превращать их в разрешения. Но принципиально разницы, где это делать,
        # нет. При текущей реализации мы и формируем список разрешений, и сразу же раздаём номера этим разрешениям.
        # Но приходится дозаполнять сами сноски.
        $e->{num} = $note->{num};
        $e->{id} = $id;
        $e->{target} = $note->{id};
        delete $e->{raw};
...
# fndef - определение сноски
# inline_fnref - использование сноски в тексте
sub fndef {
    my ($s, $glb) = @_;
    ...
    # внутри определения сноски не должно оказаться вложенного определения другой сноски
    local $glb->{infn} = 1;
